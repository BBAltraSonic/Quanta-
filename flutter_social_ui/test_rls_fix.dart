...
Future<void> testRLSFix() async {
  ...
  try {
    ...
    if (posts.isNotEmpty) {
      ...
      final statusResult = await supabase.rpc('get_post_interaction_status', params: {
        'target_post_id': testPost.id,
      });
      ...
      if (statusResult['success']) {
        ...
        print('❌ get_post_interaction_status RPC failed: ${statusResult['error']}');
      }
      ...
      try {
        ...
        print('✅ Like toggle successful! New status: ${likeResult ? 'liked' : 'unliked'}');
        
        // Test 5: Toggle back
        print('\n5. Testing like toggle back...');
        final unlikeResult = await enhancedFeedsService.toggleLike(testPost.id);
        print('✅ Unlike toggle successful! New status: ${unlikeResult ? 'liked' : 'unliked'}');
      ...
      try {
        final interactionResult = await interactionService.toggleLike(testPost.id);
        print('✅ InteractionService like toggle successful! Status: ${interactionResult ? 'liked' : 'unliked'}');
      } catch (e) {
