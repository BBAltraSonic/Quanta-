...
class InteractionService {
  ...
  Future<bool> _toggleLikeSupabase(String postId, String userId) async {
    try {
      ...
      final statusResult = await _supabase.rpc(
  ...
  Future<void> _sharePostSupabase(
    String postId,
    String userId,
    String? message,
  ) async {
  ...
  Future<List<Comment>> _getCommentsSupabase(
    String postId,
    int limit,
    int offset,
  ) async {
  ...
  Future<Map<String, dynamic>> getPostStats(String postId) async {
    try {
      ...
      return {'likes': 0, 'saves': 0, 'shares': 0, 'comments': 0};
    } catch (e) {
