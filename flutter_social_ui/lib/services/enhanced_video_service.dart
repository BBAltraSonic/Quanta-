...
class EnhancedVideoService {
  ...
  /// Extract post ID from video URL
  String _getPostIdFromUrl(String videoUrl) {
    // Parse post ID from standardized video URL pattern
    // Expected format: /posts/{postId}/video or similar
    try {
      final uri = Uri.parse(videoUrl);
      final pathSegments = uri.pathSegments;
      
      // Look for posts/{id} pattern
      for (int i = 0; i < pathSegments.length - 1; i++) {
        if (pathSegments[i] == 'posts' || pathSegments[i] == 'post') {
      ...
      // If no clear pattern found, generate stable ID from URL
      // This is more stable than hashCode and avoids collisions
      return uri.toString().replaceAll(RegExp(r'[^a-zA-Z0-9]'), '_');
    } catch (e) {
      debugPrint('Error parsing post ID from URL $videoUrl: $e');
      // Return the base64 encoded URL as a stable fallback
      return videoUrl.replaceAll(RegExp(r'[^a-zA-Z0-9]'), '_');
    }
  }
