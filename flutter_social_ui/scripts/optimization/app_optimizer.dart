...
class AppOptimizer {
  ...
  Future<OptimizationReport> runOptimization() async {
    ScriptLogger.info('🚀 Starting app optimization analysis...');
  ...
  Future<void> _analyzeAppSize() async {
    ScriptLogger.info('📦 Analyzing app size...');
    ...
    ScriptLogger.info('✅ App size analysis completed');
  }
  ...
  Future<void> _analyzeBuildSize() async {
    ...
    if (assetsDir.existsSync()) {
      ...
      _metrics['assets_size_mb'] = (totalAssetSize / (1024 * 1024))
          .toStringAsFixed(2);

      if (totalAssetSize > 20 * 1024 * 1024) {
        // > 20MB
        _addResult(
          OptimizationResult(
            category: 'Assets',
  ...
  Future<void> _analyzeDependencies() async {
    ScriptLogger.info('📚 Analyzing dependencies...');
    ...
    ScriptLogger.info('✅ Dependencies analysis completed');
  }
  ...
  Future<void> _analyzeAssets() async {
    ScriptLogger.info('🖼️ Analyzing assets...');

    final assetsDir = Directory('assets');
    if (!assetsDir.existsSync()) {
      ScriptLogger.info('ℹ️ No assets directory found');
      return;
    }
    ...
    ScriptLogger.info('✅ Assets analysis completed');
  }

  /// Analyze code for optimization opportunities
  Future<void> _analyzeCode() async {
    ScriptLogger.info('💻 Analyzing code...');

    await _analyzeFlutterCode();
    await _analyzeUnusedCode();

    ScriptLogger.info('✅ Code analysis completed');
  }
  ...
  Future<void> _analyzeUnusedCode() async {
      ...
      if (file is File && file.path.endsWith('.dart')) {
        ...
        for (final line in lines) {
          if (line.trim().startsWith('import ')) {
            // Simple string parsing to extract import paths
    ...
    final potentiallyUnused = allFiles
        .where(
          (file) => !importedFiles.any((imported) => file.contains(imported)),
        )
        .length;
  ...
  Future<void> _analyzePerformance() async {
    ScriptLogger.info('⚡ Analyzing performance characteristics...');

    // Check for common performance issues
    await _checkPerformancePatterns();

    ScriptLogger.info('✅ Performance analysis completed');
  }
      ...
      if (file is File && file.path.endsWith('.dart')) {
        ...
        if (content.contains('build(') &&
          ...
          performanceIssues.add(
            'Async operations in build method: ${file.path}',
          );
        }
  ...
  Future<void> _generateOptimizations() async {
    ScriptLogger.info('🔧 Generating optimization recommendations...');
    ...
    ScriptLogger.info('✅ Optimization recommendations generated');
  }
  ...
  OptimizationReport _generateReport() {
    final highImpact = _results
        .where((r) => r.impact == OptimizationImpact.high)
        .length;
    final mediumImpact = _results
        .where((r) => r.impact == OptimizationImpact.medium)
        .length;
    final lowImpact = _results
        .where((r) => r.impact == OptimizationImpact.low)
        .length;
    final autoFixable = _results.where((r) => r.autoFixAvailable).length;

    ScriptLogger.info('📋 Optimization Analysis Summary:');
    ScriptLogger.info('   🔴 High Impact: $highImpact');
    ScriptLogger.info('   🟡 Medium Impact: $mediumImpact');
    ScriptLogger.info('   🟢 Low Impact: $lowImpact');
    ScriptLogger.info('   🔧 Auto-fixable: $autoFixable');
    ScriptLogger.info('   📊 Total Issues: ${_results.length}');
  ...
  Future<void> _saveReport(OptimizationReport report) async {
    ...
    final reportFile = File(
      'reports/optimization/app_optimization_$timestamp.json',
    );

    await reportFile.writeAsString(jsonEncode(report.toJson()));

    ScriptLogger.info('💾 Report saved to: ${reportFile.path}');
  }
}
...
enum OptimizationImpact { high, medium, low }
...
Future<void> main() async {
  try {
    ...
    ScriptLogger.info('✅ App optimization analysis completed!');

    if (report.highImpactIssues > 0) {
      ScriptLogger.warning('⚠️ High-impact optimizations needed!');
      exit(1);
    }

    exit(0);
  } catch (e) {
    ScriptLogger.error('❌ App optimization failed: $e', error: e);
    exit(1);
  }
}
